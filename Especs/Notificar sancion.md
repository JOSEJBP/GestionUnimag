...
Como sistema, después de que *Actualizar score de confianza* registre una reducción, debo notificar al estudiante a través del Módulo 2, informándole el motivo, los puntos restados y su nuevo score.
...
- **Módulo 2 no disponible:** el sistema debe encolar la notificación para reintento en lugar de perderla.
...
- **Error de entrega en Módulo 2:** el sistema debe registrar el fallo de notificación sin bloquear ni revertir el cambio de score, la sanción o el bloqueo ya aplicados.

## Requirements

### Functional Requirements
- **FR-001:** El sistema debe notificar al estudiante, a través del Módulo 2, cada vez que *Actualizar score de confianza* registre un cambio de score (reducción, aumento por check-out, o recuperación por período).
...
- **FR-008:** El sistema debe reintentar el envío de la notificación a través del Módulo 2 si el primer intento falla.
...
## Key Entities
- **Notificacion:** Representa el mensaje enviado al estudiante. Contiene tipo (Reducción / Aumento / Bloqueo / Desbloqueo), canal utilizado (Módulo 2), contenido, fecha de envío y estado (Enviada, Fallida, Reintentando).
...
|---|---|---|
| Módulo 2 | **Reactivo (cola)** | El envío de la notificación se publica como evento y se procesa de forma asíncrona; no debe bloquear el flujo que originó el cambio de score, la sanción o el bloqueo. Por eso el sistema puede reintentar el envío y encolarlo si el módulo no responde de inmediato. |
